        ...
        >
            <div className="w-full max-w-4xl h-[85vh] md:h-[80vh] bg-apex-900 border border-apex-700 rounded-xl shadow-2xl flex flex-col overflow-hidden animate-fadeIn">
                ...
                <div className="bg-apex-800 p-6 md:p-8 border-b border-apex-700 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <h2 id="wallet-title" className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-1">Organization Wallet</h2>
                        <div id="wallet-balance" className="flex items-baseline space-x-4">
                        ...
                        <div className="mt-2 flex items-center space-x-2">
                            <span className="px-2 py-0.5 bg-emerald-900/30 border border-emerald-900/50 text-emerald-400 text-xs uppercase font-bold rounded" role="status">Plan: Pilot</span>
                            <span className="px-2 py-0.5 bg-blue-900/30 border border-blue-900/50 text-blue-400 text-xs uppercase font-bold rounded" role="status">Auto-Topup: Off</span>
                        </div>
                    </div>
                    <div className="flex flex-row md:flex-col items-center md:items-end w-full md:w-auto justify-between md:justify-start space-x-0 md:space-y-3">
                        <button
                            className="px-6 py-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded shadow-lg shadow-emerald-900/20 text-sm transition-all"
                            aria-label="Add credits to wallet"
                        >
                            <i className="fa-solid fa-plus mr-2" aria-hidden="true"></i> Add Credits
                        </button>
                        <button
                            onClick={onClose}
                        ...
                        >
                            Close <i className="fa-solid fa-xmark ml-1" aria-hidden="true"></i>
                        </button>
                    </div>
                </div>

                {/* Navigation */}
                <div className="flex border-b border-apex-700 bg-apex-800/50 px-4 md:px-8 overflow-x-auto" role="tablist" aria-label="Wallet sections">
                    <button
                        ...
                        id="transactions-tab"
                    >
                        <i className="fa-solid fa-list-ul mr-2" aria-hidden="true"></i> Transaction History
                    </button>
                    <button
                        ...
                        id="compliance-tab"
                    >
                        <i className="fa-solid fa-shield-halved mr-2" aria-hidden="true"></i> Compliance Logs (EU AI Act)
                    </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar bg-apex-900 p-0">
                    {activeTab === 'transactions' ? (
                        ...
                        >
                            <table className="w-full text-left border-collapse min-w-[600px]" aria-label="Transaction history">
                        ...
                        >
                            <div className="p-4 bg-blue-900/10 border border-blue-900/30 rounded mb-6 flex items-start" role="note" aria-label="Compliance information">
                                <i className="fa-solid fa-circle-info text-blue-500 mt-0.5 mr-3 text-sm" aria-hidden="true"></i>
                            ...
                            {logs.slice().reverse().map((log) => (
                                <div key={log.id} className="bg-apex-800 border border-apex-700 rounded-lg overflow-hidden">
                                    ...
                                    >
                                        ...
                                        <i className={`fa-solid fa-chevron-down text-slate-500 transition-transform ${expandedLog === log.id ? 'rotate-180' : ''}`} aria-hidden="true"></i>
                                    </button>

                                    {expandedLog === log.id && (
                                        <div
                                            id={`log-details-${log.id}`}
                                            className="p-4 bg-black/30 border-t border-apex-700 font-mono text-xs text-emerald-400/80 overflow-x-auto"
                                            role="region"
