...
import { AuditEvent, AuditEventType, CREDITS_TO_EUR } from '../types';
...
const AuditLogModal: React.FC<Props> = ({ credits, logs, onClose }) => {
  ...
  const formatCurrency = (cr: number) => `~€${(Math.abs(cr) * CREDITS_TO_EUR).toFixed(2)}`;
        ...
        <div className="bg-apex-800 p-6 md:p-8 border-b border-apex-700 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                ...
                <div className="mt-2 flex items-center space-x-2">
                    <span className="px-2 py-0.5 bg-emerald-900/30 border border-emerald-900/50 text-emerald-400 text-[10px] uppercase font-bold rounded">Plan: Pilot</span>
                    <span className="px-2 py-0.5 bg-blue-900/30 border border-blue-900/50 text-blue-400 text-[10px] uppercase font-bold rounded">Auto-Topup: Off</span>
                </div>
            </div>
            ...
            {activeTab === 'transactions' ? (
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse min-w-[600px]">
                        <thead className="bg-apex-800/50 sticky top-0 text-[10px] font-bold uppercase text-slate-500 tracking-wider">
                            <tr>
                            ...
                            {logs.slice().reverse().map((log) => (
                                <tr key={log.id} className="border-b border-apex-800 hover:bg-apex-800/30 transition-colors">
                                    ...
                                    <td className="p-4 text-slate-200">
                                        ...
                                        <div className="text-[10px] text-slate-500 ml-5 font-mono">{log.id}</div>
                                    </td>
                ...
                <div className="p-4 md:p-6 space-y-4">
                    ...
                    {logs.slice().reverse().map((log) => (
                        <div key={log.id} className="bg-apex-800 border border-apex-700 rounded-lg overflow-hidden">
                            <div 
                                className="p-4 flex justify-between items-center cursor-pointer hover:bg-apex-700/50 transition-colors"
                                onClick={() => setExpandedLog(expandedLog === log.id ? null : log.id)}
                            >
                                <div className="flex flex-col md:flex-row md:items-center space-y-2 md:space-y-0 md:space-x-3">
                                    <span className="text-xs font-mono text-slate-500">{new Date(log.timestamp).toISOString()}</span>
                                    <span className="px-2 py-0.5 bg-slate-700 rounded text-[10px] font-bold text-slate-300 uppercase w-fit">{log.type}</span>
