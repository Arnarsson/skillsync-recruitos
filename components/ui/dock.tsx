...
function DockCardInner({ src, id, children, onClick, className }: DockCardInnerProps) {
  ...
  if (!src) {
    return (
      <span
        className={cn(
          "relative flex justify-center items-center z-0 overflow-hidden w-full h-full rounded-md",
          className
        )}
        onClick={onClick}
      >
  ...
  return (
    ...
    >
      ...
      <AnimatePresence>
        {id && animatingIndexes.includes(parseInt(id)) && children ? (
          <motion.div
      ...
      <AnimatePresence mode="popLayout">
        {id && !animatingIndexes.includes(parseInt(id)) ? (
          <motion.img
